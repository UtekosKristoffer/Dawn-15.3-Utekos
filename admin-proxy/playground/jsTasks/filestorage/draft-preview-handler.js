...
class ProductPreviewHandler {
  ...
  #loadInitialData() {
    try {
      ...
      console.log(productJsonElement);
  ...
  #setupFormElements() {
    ...
    this.#addToCartButton = this.#sectionElement.querySelector('[data-add-to-cart-button]');
  ...
  #updateImageGallery(vaultVariantData) {
    ...
    validMedia.forEach((media, index) => {
      ...
      img.setAttribute('data-image-id', media.id);
      ...
      thumbSlide.dataset.mediaId = String(media.id);
      thumbSlide.dataset.swiperSlideIndex = String(index);
    ...
    const updateActiveThumb = (activeMediaId) => {
      const thumbs = this.#sectionElement.querySelectorAll(
        '.product-thumbs-swiper [data-image-id]'
      );
      thumbs.forEach((el) => {
      ...
      const activeEl = this.#sectionElement.querySelector(
        `.product-thumbs-swiper [data-image-id="${activeMediaId}"]`
      );
    ...
    const thumbSlides = thumbsWrapper.querySelectorAll('[data-image-id]');
  ...
  #updateUI(shopifyVariant) {
    ...
    if (targetShopifyVariant && vaultVariantData) {
      ...
      if (this.#comparePriceElement && this.#comparePriceLabelElement) {
        ...
        console.log(shopifyVariant, vaultVariantData);
      }
  ...
  #setupTruncatedText(
    ...
    if (!(contentEl instanceof HTMLElement) || !(buttonEl instanceof HTMLElement)) return;
    // If buttonTopEl exists, it also needs a type guard if you access properties on it later
    if (buttonTopEl && !(buttonTopEl instanceof HTMLElement)) return;

    const fullHtml = contentEl.innerHTML.trim();

    if (!fullHtml) {
      buttonEl.style.display = 'none';
      if (buttonTopEl instanceof HTMLElement) buttonTopEl.style.display = 'none';
      return;
    }
    ...
    let isExpanded = false;
    const updateState = () => {
      contentEl.innerHTML = isExpanded ? fullHtml : truncatedHtml; // <-- Changed 'fullText' to 'fullHtml'
      ...
      if (buttonTopEl instanceof HTMLElement) {
    ...
    [buttonEl, buttonTopEl].forEach((btn) => {
      if (btn instanceof HTMLElement) btn.setAttribute('aria-controls', contentId);
    });

    const newButtonEl = buttonEl.cloneNode(true);

    if (buttonEl.parentNode instanceof HTMLElement) {
      buttonEl.parentNode.replaceChild(newButtonEl, buttonEl);
    }
    const newButtonTopEl = buttonTopEl instanceof HTMLElement ? buttonTopEl.cloneNode(true) : null;
    if (buttonTopEl instanceof HTMLElement && newButtonTopEl instanceof HTMLElement) {
      if (buttonTopEl.parentNode instanceof HTMLElement) {
  ...
  init() {
    ...
    if (quantityInput && this.#variantIdInput) {
    ...
    // === Initially loading first variantimage if available ===
    ...
    Promise.resolve().then(() => {
      this.#initializeSwiper();
      console.log('[Swiper] Delayed Init complete (microtask).');
    });
  }
}
