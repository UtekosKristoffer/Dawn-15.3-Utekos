...
if (!customElements.get('product-preview-handlers')) {
  customElements.define(
    'product-preview-handler',
    class ProductPreviewHandlers extends HTMLElement {
      #sectionElement = undefined;
      #loadInitialData = undefined;
      #setupFormElements = undefined;
      #initializeProductSwapUtility = undefined;
      #originalProductData = undefined;
      #vaultVariantData = undefined;
      #vaultVariantIdInput = undefined;
      #vaultVariantHandler = undefined;
      #activeVaultVariantHandler = undefined;
      #addToBagButton = undefined;
      #handleOptionValueChange = undefined; 
      #onOptionChanged = null;
      #onOptionChangedDebounced = undefined;
      ...
      constructor() {
        super();
        this.#sectionElement = this.#sectionElement.bind(this);
        this.#loadInitialData = this.#loadInitialData.bind(this);
        this.#setupFormElements = this.#setupFormElements.bind(this);
        this.#vaultvariantId = this.#vaultvariantId.bind(this);
        this.#selectedProductData = this.#parseSelectedProduct.bind(this) 
        this.#priceElement = this.#priceElement.bind(this);
        this.#priceLabelElement = this.#priceLabelElement.bind(this);
        this.#comparePriceElement = this.#comparePriceElement.bind(this);
        this.#comparePriceLabelElement = this.#comparePriceLabelElement.bind(this);
        this.#addToBagButton = this.#addToBagButton.bind(this);
        this.#quantityInput = this.#quantityInput.bind(this);
        this.#onOptionChanged = this.#onOptionChanged.bind(this);
        this.#onOptionChangedDebounced = debounce(this.#onOptionChanged.bind(this), 300);
      }
      ...
      connectedCallback() {
        ...
        this.initializeProductSwapUtility();
        const productUrl = target.dataset.spotlightedVariantUrl || this.pendingRequestUrl || this.dataset.url;
        this.pendingRequestUrl = productUrl;
        const shouldSwapProduct = this.dataset.url !== productUrl;
        const shouldFetchFullPage = this.dataset.updateUrl === 'true' && shouldSwapProduct;
        this.#vaultvariantId = this.#vaultvariantId.bind(this);
        this.#originalProductData = this.#parseShopifyOptions.bind(this) 
        this.#priceElement = this.#priceElement.bind(this);
        this.#priceLabelElement = this.#priceLabelElement.bind(this);
        this.#comparePriceElement = this.#comparePriceElement.bind(this);
        this.#comparePriceLabelElement = this.#comparePriceLabelElement.bind(this);
    ...
    this.#originalProductData = this.#parseShopifyOptions(this.#originalProductData);
    this.init();
  

 this.addEventListener('change', this.#onOptionChangedDebounced);
  }
    #loadInitialData(){
    try {
      ...
      this.#originalProductData = JSON.parse(rawProductJson);
      ...
      this.#vaultVariantData = JSON.parse(vaultVariantsJsonScript.textContent);

      return !(
        !this.#originalProductData ||
        typeof this.#shopifyProductData !== 'object' ||
        !this.#originalProductData.variants?.length ||
        !this.#vaultVariantData
      );
    } catch (e) {
      this.#originalProductData = null;
      this.#vaultVariantData = null;
      return false;
    }
  }
    ...
    this.#vaultvariantId = this.#productForm.querySelector('[data-variant-id-input]');
    this.#addToBagButton = this.#productForm.querySelector('[product-preview__add-to-cart-button');
    }

    #setupSectionElements(){
      this.#swatchValueInput = this.#productForm.querySelector(
    ...
    this.#quantityInput = this.#sectionElement.querySelector('.quantity__input');
    
    return true;
    }
this.addEventListener('change', this.#handleOptionValueChange);
        this.onVariantChangeUnsubscriber = subscribe(
          PUB_SUB_EVENTS.optionValueSelectionChange,
          this.handleOptionValueChange.bind(this)
        );

     const initialVariantId = this.#vaultvariantId.value;
    this.#activeVariant = this.#allVaultVariantData[initialVariantId];
        if (this.#activeVaultVariantHandler) {
      ...
      handleOptionValueChange({ data: { event, target, selectedOptionValues } }) {
        ...
        const productUrl = target.dataset.Url || this.pendingRequestUrl || this.dataset.url;
        this.pendingRequestUrl = productUrl;
        const shouldSwapProduct = this.dataset.url !== productUrl;
        const shouldFetchFullPage = this.dataset.updateUrl === 'true' && shouldSwapProduct;
      ...
      handleSwapProduct(productUrl, updateFullPage) {
        return (html) => {
          ...
          const selector = updateFullPage ? "product-preview-handler[id^='Product-Preview-Main']" : 'product-preview-handler';
