class StaticShowcaseHandler {
  ...
  initQuickViewModal() {
    ...
    const openModal = (url) => {
      if (!url || url === '#') {
        // Ikke åpne modal hvis URL mangler
        console.warn(
          'StaticShowcaseHandler: Quick view URL is missing or invalid for a plus button.'
        );
        return;
      }
    ...
    this.sectionElement.querySelectorAll('.plus-button-showcase').forEach((btn) => {
      if (!btn.dataset.modalListenerAttachedSc) {
        // Unikt dataset-attributt for denne handleren
        btn.addEventListener('click', function () {
          openModal(this.dataset.productUrl);
        });
    ...
    if (!document.body.dataset.globalEscListenerAttachedModalSc) {
      // Unikt dataset-attributt
...
document.addEventListener('DOMContentLoaded', () => {
  document
    .querySelectorAll('.static-product-showcase-wrapper[data-section-id]')
    .forEach((sectionEl) => {
