...
import CART_FROM_ASYNCED_COOKIESQUERY from "@/ServerFunctions/Cart/CartFromCookiesAndCacheOnServer";
import { ContextCartProcess } from "@/Components/Cart/CartProcessClient";
import { cartStore, useCartOpen, useCartPending } from "@/Lib/State/CartModel";

export default function CartDrawer() {
  const pending = useCartPending() > 0;
  const { data: cart } = useQuery(CART_FROM_ASYNCED_COOKIESQUERY);
  ...
  return (
    <Drawer
      open={useCartOpen()}
      onOpenChange={(open) => cartStore.send({ type: open ? "OPEN" : "CLOSE" })}
      direction="right"
    >
      <DrawerTrigger asChild>
        <Button
          ...
          className="relative"
          onClick={() => cartStore.send({ type: "OPEN" })}
        >
        ...
        {!isCartEmpty && cart && (
          <DrawerFooter className="border-t">
            ...
            <a
              ...
              className={`mt-4 block w-full rounded-md bg-primary py-3 text-center text-sm font-medium text-primary-foreground ${pending ? "pointer-events-none opacity-50" : ""}`}
            >
              Gå til kassen
            </a>
          </DrawerFooter>
